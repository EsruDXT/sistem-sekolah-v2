...
@section('title', $title ?? 'Detail Data Guru')
...
@section('content')
    ...
    <div class="mt-3 border border-[#E5E3DB] bg-white">
        ...
        <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
            ...
            <a href="{{ route('teachers.edit', ['id' => $teacher['id'] ?? 1]) }}"
        ...
        <dl class="divide-y divide-[#EFEDE6] text-sm">
            ...
            <div class="flex justify-between px-8 py-4">
                ...
                <dd class="font-medium text-[#16213A]">{{ $teacher['phone_number'] ?? $teacher['phone'] ?? '' }}</dd>
        ...
        <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
            ...
            <form action="{{ route('teachers.destroy', ['id' => $teacher['id'] ?? 1]) }}" method="POST" onsubmit="return confirm('Hapus data guru ini dari buku induk?')">
