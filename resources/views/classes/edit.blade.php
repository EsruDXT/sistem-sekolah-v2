...
@section('title', $title ?? 'Ubah Data Kelas')
...
@section('content')
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">Memperbarui catatan untuk kelas <span class="font-medium text-[#16213A]">{{ $class['name'] ?? '' }}</span>.</p>

    </div>



    <form action="{{ route('classes.update', ['id' => $class['id'] ?? 1]) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        ...
        <div>
            ...
            <select id="major_id" name="major_id"
                ...
                @foreach ($majors as $major)
                    <option value="{{ $major['id'] }}" {{ ($class['major_id'] ?? $class['major'] ?? '') == $major['id'] || ($class['major'] ?? '') == $major['code'] ? 'selected' : '' }}>
        ...
        <div>
            ...
            <select id="teacher_id" name="teacher_id"
                ...
                @foreach ($teachers as $teacher)
                    <option value="{{ $teacher['id'] }}" {{ ($class['teacher_id'] ?? $class['homeroom_teacher'] ?? '') == $teacher['id'] || ($class['homeroom_teacher'] ?? '') == $teacher['name'] ? 'selected' : '' }}>
                        {{ $teacher['name'] }}
                    </option>
                @endforeach
